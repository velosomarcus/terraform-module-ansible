...
# Optional variables - they already have a default value, but you can change them as well.
variable "galaxy_role_file" {
  description = "Your ansible galaxy requirements file (full path)."
  type        = string
  default     = ""
}
...
variable "ansible_host_key_checking" {
  description = "Ansible enables host key checking by default. Set this to false if you want to avoid host key checking by the underlying tools Ansible uses to connect to the host."
  type        = bool
  default     = false
}
variable "extra_vars" {
...
variable "vault_pass_file" {
  description = "The path of a file containing the password used by Ansible Vault."
  type        = string
  default     = ""
}
